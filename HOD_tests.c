...
Spline *prep_nfw_cumsums(double halo_mass, int bins, Spline *variance, Parameters *params) {
	...
	//Spline *test = input_spline_values(bins, Rs, cumsums, MEASURED);
	//print_spline(test, r_binInfo);
	//exit(0);
	/*
	char out_test[100];
	sprintf(out_test, "%s/data/cumsum.dat", home_directory);
	print_spline_file(test, r_binInfo, out_test);
	*/
	
	Spline *toReturn = input_spline_values(bins, cumsums, Rs, MEASURED);
	/*
	double cs_min = toReturn->splineInfo->xmin;
	double cs_max = toReturn->splineInfo->xmax;
	printf("cs min: %le, rmin: %le, cs max: %le, rmax: %le, test: %le \n", cs_min, cs_max, splint_generic(toReturn, cs_min), splint_generic(toReturn, cs_max), splint_generic(toReturn, 0.5));
	print_spline(toReturn, prep_bins(cs_min, cs_max, 100, NORMAL_BIN));
	*/
...
Catalogue *HOD_satellites(Catalogue *current_catalogue, Spline *variance, Parameters *params) {
	...
	Spline **NFW_cumsums = nfw_cumsums_catalogue(mass_binInfo, 200, variance, params);
	//rvir, rs, cdm for each mass bin
	HOD_Parameters **HOD_parameters_catalogue = HOD_params_catalogue(mass_binInfo, variance, params);
	*/
	
	//ONE MASS TESTS
	Spline *NFW_cumsums_oneMass = prep_nfw_cumsums(5e13, 300, variance, params);	
	HOD_Parameters *HOD_parameters_oneMass = HOD_params(5e13, variance, params);
	printf("cdm: %lf, rvir: %lf, rs: %lf\n",HOD_parameters_oneMass->cdm, HOD_parameters_oneMass->rvir, HOD_parameters_oneMass->rs); 	
	...
	for (int i = 0; i < current_catalogue->particle_no; i++) {
		...
		//velocity modifier to reflect halo concentration
		/*
		...
		*/
		...
		for (int j = 0; j < no_satellites[i]; j++) {
			...
			r_sat = splint_generic(NFW_cumsums_oneMass, randomDouble(0.0, 1.0));
			...
			//sigma_sq = ((G*current_catalogue->particles[i]->mass)/(3.0*HOD_parameters_catalogue[cur_massBin]->rvir))*cdm_vel_par;
			
			//sigma_sq = ((G*current_catalogue->particles[i]->mass)/(3.0*HOD_parameters_oneMass->rvir))*cdm_vel_par;
			//sigma = sqrt(sigma_sq)*10.0;
			
			sigma = 5.0;
			velDisp = gsl_ran_gaussian(gsl_ran_r, sigma);
			//velDisp = 0.0;
			mean_velDisp += velDisp;
	...
	mean_velDisp /= (double) satellite_no;
	printf("\n mean r of sats: %lf, sigma r sat: %lf, mean RMS virial velocity: %lf \n", mean_r_sat, sqrt(mean_r_sq_sat - pow(mean_r_sat, 2.0)), mean_velDisp); 
...
int full_catalogue_HOD(char DM_catalogue_in[], char HOD_catalogue_out[], char input_format[], Parameters *params) {
	...
	catalogue_to_file(current_catalogue, HOD_catalogue_out);
	free_catalogue(current_catalogue);
	return 0;
}
...
//with comments for report
Catalogue *HOD_satellites(Catalogue *current_catalogue, Spline *variance, Parameters *params) {

	double mean_N_sats, sum_weights, sum_weights_sq, cos_theta_sat, phi_sat, theta_sat;
 	double NFW_cumsum, r_sat, x_sat, y_sat, z_sat, vx_sat, vy_sat, vz_sat, sigma_sq;
	double sigma, c, velDisp, cdm_vel_par;
	sum_weights = sum_weights_sq = mean_r_sat = mean_r_sq_sat = 0.0;	
	double *mass_lims = mass_min_max(current_catalogue);
	double Mmin = mass_lims[0];
	double Mmax = mass_lims[1];
	int massBins = 300;
...
//without comments for report
Catalogue *HOD_satellites(Catalogue *current_catalogue, Spline *variance, Parameters *params) {
	double mean_N_sats, sum_weights, sum_weights_sq, cos_theta_sat, phi_sat, theta_sat;
 	double NFW_cumsum, r_sat, x_sat, y_sat, z_sat, vx_sat, vy_sat, vz_sat, sigma_sq;
	double sigma, c, velDisp, cdm_vel_par;
	int satellite_no, satellite_index, cur_massBin;
	sum_weights = sum_weights_sq = mean_r_sat = mean_r_sq_sat = 0.0;
	satellite_no = satellite_index = 0;		
	double *mass_lims = mass_min_max(current_catalogue);
	int massBins = 300;
	BinInfo *mass_binInfo = prep_bins(mass_lims[0], mass_lims[1], massBins, LOG_BIN);	
	Spline **NFW_cumsums = nfw_cumsums_catalogue(mass_binInfo, 200, variance, params);
	HOD_Parameters **HOD_parameters_catalogue = HOD_params_catalogue(mass_binInfo, variance, params);		
	...
	double weighted_shotnoise_sats = volume*sum_weights_sq / pow(sum_weights, 2.0);
	Particle **satellites = malloc((size_t)satellite_no * sizeof(*satellites));	
